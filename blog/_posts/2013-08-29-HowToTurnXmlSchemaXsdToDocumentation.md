...
[![]({{ site.blogImagesFolder }}{{ page.name }}/SampleOutput.png)]({{ site.blogImagesFolder }}{{ page.name }}/SampleOutput-big.png)
You can download view [sample HTML output]({{ site.blogImagesFolder }}{{ page.name }}/SampleOutputOfMsXslTool.html).  
To make things easier, I wrote a [PowerShell script]({{ site.blogImagesFolder }}{{ page.name }}/ConvertAllXsdToHtml.ps1) that find all the XSD files in the folder where it's placed and convert them all to HTML. It assumes that both _MSXSL.exe_ and _xs3p.xsl_ are in the same folder.
...
This [ZIP file]({{ site.blogImagesFolder }}{{ page.name }}/TurnXmlSchemaToHtml.zip) include all three files: PowerShell script, _MSXSL.exe_ and style sheet _xs3p.xsl_.
